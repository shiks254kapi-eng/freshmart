...
import { Plus, X, Star, Bike } from 'lucide-react'
...
export default function Riders() {
  ...
  const statusBadge = (status) => {
    ...
    return <span style={{ fontSize: 11, fontWeight: 700, background: s.bg, color: s.color, padding: '3px 10px', borderRadius: 10 }}>{s.label}</span>
  }
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => setShowModal(true)}><Plus size={16}/> Add Rider</button>
      </div>

      {/* SUMMARY CARDS */}
      <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(3,1fr)', marginBottom: 20 }}>
        <div className="stat-card">
          <div className="stat-label"><Bike size={14}/> Active Now</div>
          <div className="stat-value">{activeRiders}</div>
          <div className="stat-change up">of {riders.length} riders</div>
        </div>
        <div className="stat-card">
            ...
            <div className="card-body">
              <div className="flex-between mb-16">
                <div className="flex gap-12">
                  <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'var(--g5)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 15, color: 'var(--g1)' }}>
                    {r.name.split(' ').map(n => n[0]).join('').slice(0,2)}
                  </div>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 14 }}>{r.name}</div>
                    <div style={{ fontSize: 12, color: 'var(--text3)' }}>{r.phone} • {r.zone}</div>
                    <div style={{ color: 'var(--accent)', fontSize: 13, marginTop: 2 }}>{stars(r.rating)} <span style={{ color: 'var(--text3)', fontSize: 11 }}>{r.rating}</span></div>
                  </div>
                </div>
                {statusBadge(r.status)}
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                <div style={{ background: 'var(--g5)', borderRadius: 8, padding: '8px 10px', textAlign: 'center' }}>
                  <div style={{ fontSize: 10, color: 'var(--text3)', fontWeight: 600 }}>DELIVERIES</div>
                  <div style={{ fontSize: 18, fontWeight: 700, fontFamily: 'var(--serif)' }}>{r.deliveries_today}</div>
                </div>
                <div style={{ background: 'var(--g5)', borderRadius: 8, padding: '8px 10px', textAlign: 'center' }}>
                  <div style={{ fontSize: 10, color: 'var(--text3)', fontWeight: 600 }}>AVG TIME</div>
                  <div style={{ fontSize: 18, fontWeight: 700, fontFamily: 'var(--serif)' }}>{r.avg_time || '—'} <span style={{ fontSize: 11 }}>min</span></div>
                </div>
                <div style={{ background: 'var(--g5)', borderRadius: 8, padding: '8px 10px', textAlign: 'center' }}>
                  <div style={{ fontSize: 10, color: 'var(--text3)', fontWeight: 600 }}>EARNED</div>
                  <div style={{ fontSize: 18, fontWeight: 700, fontFamily: 'var(--serif)' }}>{r.earnings_today}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <select value={form.zone} onChange={e => setForm({...form,zone:e.target.value})}>
                ...
                {['Westlands','Kilimani','Karen','Kasarani','Eastleigh','Parklands','Ngong Road','South B','South C'].map(z => <option key={z}>{z}</option>)}
              </select>
            </div>
            <div className="flex gap-8" style={{ justifyContent: 'flex-end', marginTop: 8 }}>
              <button className="btn btn-outline" onClick={() => setShowModal(false)}>Cancel</button>
              <button className="btn btn-primary" onClick={addRider}>Add Rider</button>
            </div>
          </div>
        </div>
      )}
